...
let expr_parser =
  ...
  let num =
    ...
    | "" -> nums
    | _ -> hex_nums)
    >>= fun n -> return (Const (String.concat "" [ s; p; n ]))
  in
  let reg =
    word
    >>= fun x ->
...
let code_line_parser =
  ...
  let inst =
    ...
    then trim word >>= fun l -> return (Command (Jmp (cmd, Label l)))
      ...
      | 1 when is_arg1 @@ command cmd -> return (Command (Args1 (cmd, List.hd exprs)))
...
let%test _ =
  test_p expr_parser show_expr "0 + 0x0" = "(Add ((Const \"0\"), (Const \"0x0\")))"
;;

let%test _ =
  test_p expr_parser show_expr "0 + rax" = "(Add ((Const \"0\"), (Reg \"RAX\")))"
;;

let%test _ =
  test_p expr_parser show_expr "0 + var" = "(Add ((Const \"0\"), (Var \"var\")))"
;;
...
let%test _ =
  test_p code_line_parser show_code_section "ret" = "(Command (Args0 (Mnemonic \"RET\")))"
;;
...
let%test _ = test_p sec_parser show_ast "section .code ret ret" = ": end_of_input"

let%test _ =
  match parse_string ~consume:All sec_parser "section .code inc rax inc rax" with
  | Ok v ->
    v
    = Code
        [ Command (Args1 (Mnemonic "INC", Reg "RAX"))
...
let%test _ =
  ...
  | Ok v ->
    v
    = Data
        [ Variable ("a", DataType "DD", [ "1" ]); Variable ("b", DataType "DD", [ "2" ]) ]
  | _ -> false
;;

let%test _ =
  match parse_string ~consume:All parser "section .data a dd 1 section .text ret" with
  | Ok v ->
    v
    = [ Data [ Variable ("a", DataType "DD", [ "1" ]) ]
      ; Code [ Command (Args0 (Mnemonic "RET")) ]
