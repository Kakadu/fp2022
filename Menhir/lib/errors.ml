...
(* TEMPORARY V�rifier que les messages d'erreur sont standardis�s au
   maximum, localis�s au maximum. Supprimer autant de fonctions que
...
let get_filename () = get_initialized_ref filename
let get_filemark () = !filemark
...
let errorp v message = error [ Positions.position v ] message
...
let warning positions message = printN positions (Printf.sprintf "Warning: %s" message)
let errors () = !errors
